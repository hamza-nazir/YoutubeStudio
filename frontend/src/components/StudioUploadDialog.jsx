...
const StudioUploadDialog = () => {

  const kidsSectionRef = useRef(null);
const {setSnackBarMessage, setSnackBarOpen,}=useContext(myContext)
const [isDraft,setIsDraft]=useState(true);
  const [videoData,setVideoData]=useState({title:'',description:'',category:'',moderation:'',sortBy:'',visibility:'',tagsString:'',forKids:null,restrictOverEighteen:null,comments:true,showLikeCount:false});
  const {dialogOpen,setDialogOpen}=useContext(myContext);
...
const uploadVideoFun = async (e) => {
  ...
  if (!currentUser?._id) return console.log("Error");
  ...
  tempVideo.onloadedmetadata = async () => {
    ...
    try {
      ...
      setOriginalVideoName(response.data.originalVideoName);
      setVideoData((prev) => ({
        ...prev,
        title: response.data.originalVideoName,
      }));
  ...
  const copyFun=()=>{
    const copiedLink=`https://localhost:5173/videos/${videoLink}`;
...
useEffect(() => {
  ...
  const timer = setTimeout(() => {
    const combinedData = { ...videoData, videoLink ,isDraft };
  ...
  <>
   ...
   <Dialog ref={contentRef} disableEnforceFocus={true}   open={dialogOpen}  sx={{"& .MuiDialog-paper": { height: "600px",borderRadius:'20px'}}}  fullWidth maxWidth="md"  >
...
<CloseIcon />
           ...
           {
           !otherPage&&(
          <DialogContent  dividers className="mt-2 text-center">
         
          
         
        <input onChange={uploadVideoFun} type="file" name="videoUpload" className="d-none" id="videoUpload" /> 
       
       {!uploadStart&&(
        <label htmlFor="videoUpload" className="mt-4  p-4 rounded-circle text-center" style={{backgroundColor:'#F9F9F9'}}>
           <GoUpload style={{fontSize:'70px',color:'#909090'}}  />
          </label>
        )}
  ...
  {uploadStart&&(
<label id="uploading-animation-circle" className="mt-4 p-4 rounded-circle text-center"  style={{ backgroundColor: "#F9F9F9" }}>
  <GoUpload    id="uploading-animation-rocket"    className="upload-anim"     style={{ fontSize: "70px", color: "#909090" }}/>
</label>
        ...
        <div className="mt-5">Drag and drop video files to upload</div>   
      <p className="text-muted msmall-text">Your videos will be private until you publish them</p>
      {error&&( <div className="mb-3 p-2"><CiWarning className="fs-3 text-danger me-3"/>Invalid file format. <span className="text-primary ">Learn more</span></div>   )}

          <label htmlFor="videoUpload">
            {!uploadStart&&(
              <span  className="border-0 rounded-4 bg-dark text-white msmall-text p-2" style={{cursor:'pointer'}}>Upload Video</span>
            )}
      ...
      <div className="mt-5"></div>
          ...
          <div className="msmall-text">By submitting your videos to YouTube, you acknowledge that you agree to YouTube's <span className="text-primary">Terms of Service</span> and <span className="text-primary">Community Guidelines.</span></div>
            <div className="msmall-text">Please make sure that you do not violate others' copyright or privacy rights. <span className="text-primary">Learn more</span></div>
          </DialogContent>
        )}
        ...
        {/* Step number 1 */}
              ...
              {activeStep+1==1&&(
               ...
               <div className=" d-flex justify-content-between align-items-center">
                ...
                <div className="border pt-1 pb-1 ps-2 pe-2 rounded-4" style={{backgroundColor:'#E5E5E5'}}><span  className="fw-semibold">Reuse Details</span></div>
               </div>
...
<button className="border-0 rounded-4 py-1 px-2 mb-2 mt-2 fw-semibold " style={{fontSize:'14px'}} onClick={()=>setShowMore(false)}>Show less</button>
        ...
        {showMore&&(
          ...
          <div className="mt-2">
          <input className="w-100 rounded-2 border p-2" placeholder="Add tag" type="text" name="tagsString" value={videoData.tagsString}  onChange={handleChange} />
...
<Select  value={videoData.category} name="category"onChange={handleChange}  labelId="demo-simple-select-label"  id="demo-simple-select"  label="Category"
  sx={{
    ...
    <label htmlFor="showViewersLikeTheVideo" >Show how many viewers like this video</label>
              ...
              <div>
               <div className="scroll-video-box">
                <div className="">
                  ...
                  <video  controls muted style={{width:'100%'}} src={videoUrl}  />
                ...
                <div className="">
                  ...
                  <div className="d-flex justify-content-between">
                    <div>
                       ...
                       <div className="text-primary" style={{fontSize:'14px'}} >{`https://localhost:5173/videos/${videoLink}`}</div>
                  ...
                  <div className="text-muted fw-semibold xsmall-text mt-1">Filename</div>
                    <div className="fw-semibold" style={{fontSize:'14px'}}>{originalVideoName||`lorem ispum`}</div>
                ...
                <div className=" d-flex justify-content-center align-items-center" style={{height:'300px'}}>
                    ...
                    <CircularProgress size={120}   sx={{ color: "black" }}   thickness={1}  variant="determinate" value={progress} />
        ...
        {/* Step number 2 */}
          ...
          <Box sx={{ display:'flex',justifyContent:'flex-end',  p: 2 }}>
            ...
            <Button onClick={activeStep === steps.length - 1 ?()=>submitReq(videoLink) : handleNext}    className="text-dark bg-dark text-white">
...
export default StudioUploadDialog;
