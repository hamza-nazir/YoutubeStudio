...
function WatchNav({ handleToggle }) {
  ...
  const {  currentUser,dialogOpenWithLogin,setDialogOpenWithLogin} =useContext(myContext);
...
const serachFun=()=>{
  ...
  navigate(`/result?search_query=${search}`);
}
...
<>
  ...
  <FaBars onClick={handleToggle} className='fs-2'  />
            ...
            <input type="text " className='search-text-nav  ps-2' placeholder='Search' style={{color:'black'}} onChange={(e)=>{setSearch(e.target.value)}} value={search} name="search" id="" />
